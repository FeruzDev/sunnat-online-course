...
const PersonalCourses = (props) => {
    ...
    const postBook = (data) => {
        ...
        newData.append("content", document.getElementById('n13').value)
        newData.append(    "avtor", localStorage.getItem("email"))
        newData.append(            "date" , date  )
        newData.append("file", forFile)
        newData.append("src", forFile2)
        axios.post(API_PATH + "items/course/create",  newData)
            .then(res => {
                // console.log(res)
                getBooks()
                setModal(false)
                                ...
                                books?.filter(item => item.avtor === localStorage.getItem("email")).map(item =>(
                                    <div className="col-md-4">
                                        <div className="video-boxes">
                                            ...
                                            <div>
                                                <Link to={`/detail-course/${item.id}`}  >{item.name}</Link>
                                            </div>
                                        </div>
                                    </div>
                                ))
                                ...
                                books?.filter(item => item.avtor === localStorage.getItem("email")).map(item =>(
                                    <div className="col-md-12">
                                        <div className="mini-video-boxes">
                                            ...
                                            <div>
                                                <p className="text-white" >{item.date}</p>
                                                <Link to={`/detail-course/${item.id}`}  >{item.name.length > 30  ? item.name.slice(0, 22) + "...."   : item.name}</Link>
                                            </div>
                                        </div>
                                    </div>
                                ))
            ...
            <Modal isOpen={modal} toggle={toggle} className={props.className}>
                ...
                <ModalBody>
                    ...
                    <input
                        ...
                        id="n1"
                    />
                    <label className="mt-3">Qo'llanma haqida ma'lumot</label>
                    ...
                    <textarea
                        ...
                        id="n13"
                    />
                    <label className="mt-3">Videoning sarlavha rasmi</label>
                    <input
                        name="src"
                        className="form-control "
                        type="file"
                        ...
                        id="n5"
                    />
                    <label className="mt-3">Videoning kontenti</label>
                    <input
                        name="src"
                        className="form-control "
                        type="file"
