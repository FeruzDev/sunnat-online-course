...
const PersonalBooks = (props) => {
    ...
    const postBook = (data) => {
        ...
        axios.post(API_PATH + "items/books/create",   newData    )
            .then(res => {
                console.log(res)
                getBooks()
                setModal(false)
    ...
    const getBooks = () => {
        axios.get(API_PATH + `items/books?filter[avtor][_eq]=${localStorage.getItem("email")}`)
            .then(res => {
                                ...
                                books?.filter(item => item.avtor === localStorage.getItem("email")).map(item =>(
                                    <div className="col-md-12">
                                        <div className="mini-boxes">
                                           ...
                                           <div>
                                               <p className="text-white" >{item.date}</p>
                                               <Link to={`/detail/${item.id}`}  >{item.name.length > 30  ? item.name.slice(0, 22) + "...."   : item.name}</Link>
                                           </div>
                                        </div>
                                    </div>
                                ))
